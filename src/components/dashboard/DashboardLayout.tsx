...
const navItems: NavItem[] = [
  ...
  { label: "Mensalidades", href: "/dashboard/mensalidades", icon: Receipt, roles: ["admin", "contador"] },
...
const DashboardLayout = () => {
  ...
  const roleBadgeColor: Record<UserRole, string> = {
    ...
    contador: "bg-primary/10 text-primary",
    emissor: "bg-accent/10 text-accent",
  };
              ...
              <>
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                    openSubmenu === item.label ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"
                  }`}
                >
                  <item.icon size={18} />
                  {sidebarOpen && (
      ...
      {sidebarOpen && (
        <div className="p-4 border-t border-border">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-full bg-hero-gradient flex items-center justify-center text-primary-foreground text-sm font-bold">
              {user.nome.charAt(0)}
            </div>
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      {/* Mobile overlay */}
      {mobileSidebarOpen && (
        <div className="fixed inset-0 bg-foreground/50 z-40 lg:hidden" onClick={() => setMobileSidebarOpen(false)} />
      )}
      ...
      {/* Mobile sidebar */}
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-64 bg-card border-r border-border transition-transform duration-300 lg:hidden ${
      ...
      <div className={`transition-all duration-300 ${sidebarOpen ? "lg:ml-64" : "lg:ml-16"}`}>
        ...
        <header className="sticky top-0 z-30 bg-card/80 backdrop-blur-md border-b border-border h-16 flex items-center px-4 gap-4">
          <button
            onClick={() => { if (window.innerWidth < 1024) setMobileSidebarOpen(!mobileSidebarOpen); else setSidebarOpen(!sidebarOpen); }}
          ...
          <div className="flex-1 flex items-center gap-3">
            <div className="relative hidden sm:block max-w-sm w-full">
              ...
              <input
                type="text"
                placeholder="Buscar..."
                className="w-full h-9 pl-9 pr-3 rounded-lg border border-border bg-muted/50 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              />
            </div>
          </div>

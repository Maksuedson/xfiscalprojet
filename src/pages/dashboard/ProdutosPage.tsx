...
const ProdutosPage = () => {
  ...
  const filtered = produtos.filter((p) => p.nome.toLowerCase().includes(search.toLowerCase()) || p.ncm.includes(search));
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
              <div className="sm:col-span-2 space-y-2"><Label>Descrição</Label><Input /></div>
              <div className="space-y-2"><Label>NCM</Label><Input /></div>
              <div className="space-y-2"><Label>CFOP</Label><Input /></div>
              <div className="space-y-2"><Label>Unidade</Label><Input placeholder="UN, KG, CX..." /></div>
              <div className="space-y-2"><Label>Valor Unitário</Label><Input type="number" /></div>
            </div>
            <div className="flex justify-end gap-3 mt-4">
              <Button variant="outline" onClick={() => setOpen(false)}>Cancelar</Button>
              <Button variant="hero" onClick={() => { toast.success("Produto cadastrado!"); setOpen(false); }}>Salvar</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <DataTable
        columns={[
          { key: "nome", header: "Descrição", render: (r) => <div className="flex items-center gap-2"><Package size={16} className="text-primary" /><span className="font-medium">{r.nome}</span></div> },
          { key: "ncm", header: "NCM" },
          { key: "cfop", header: "CFOP" },
          { key: "un", header: "UN" },
          { key: "valor", header: "Valor Unit." },
